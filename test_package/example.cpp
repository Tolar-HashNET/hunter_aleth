...
/// This file is here just to verify everything is compiling since we are
/// using precompiled cpp_ethereum
...
using dev::bigint;
using dev::bytes;
using dev::c_lineWidth;
using dev::fromHex;
using dev::h256;
using dev::h256s;
using dev::LoggingOptions;
using dev::setDefaultOrCLocale;
using dev::toHex;
using dev::u256;
using dev::eth::Account;
using dev::eth::AlethErrors;
using dev::eth::BlockHeader;
using dev::eth::Instruction;
using dev::eth::Network;
using dev::eth::NoProof;
using dev::eth::NoReward;
using dev::eth::OnOpFunc;
using dev::eth::StandardTrace;
...
int64_t maxBlockGasLimit() {
    static int64_t limit =
        dev::eth::ChainParams(genesisInfo(Network::MainNetwork)).maxGasLimit.convert_to<int64_t>();
    return limit;
}
...
int main(int argc, char** argv) {
    dev::FixedHash<32> bla;

    setDefaultOrCLocale();
    string codeFile;
    Mode mode = Mode::Statistics;
    dev::eth::State state(0);
    ...
    addGeneralOption(
        "timestamp",
        po::value<int64_t>()->default_value(0)->value_name("<timestamp>")->notifier([&](int64_t _t) {
          blockHeader.setTimestamp(_t);
        }),
        "<n> Set timestamp");

    po::options_description allowedOptions("Usage ethvm <options> [trace|stats|output|test]");
    allowedOptions.add(dev::eth::vmProgramOptions(c_lineWidth))
        .add(networkOptions)
        .add(optionsForTrace)
    ...
    std::vector<string> unrecognisedOptions =
    ...
    if (vm.count("mnemonics")) st.setShowMnemonics();
    if (vm.count("flat")) styledJson = false;
    if (vm.count("sender")) sender = vm["sender"].as<Address>();
    if (vm.count("origin")) origin = vm["origin"].as<Address>();
    if (vm.count("gas")) gas = vm["gas"].as<u256>();
    if (vm.count("gas-price")) gasPrice = vm["gas-price"].as<u256>();
    if (vm.count("author")) blockHeader.setAuthor(vm["author"].as<Address>());
    if (vm.count("difficulty")) blockHeader.setDifficulty(vm["difficulty"].as<u256>());
    if (vm.count("gas-limit"))
        blockHeader.setGasLimit((vm["gas-limit"].as<u256>()).convert_to<int64_t>());
    if (vm.count("value")) value = vm["value"].as<u256>();
    if (vm.count("network")) {
    ...
    if (vm.count("input")) data = fromHex(vm["input"].as<string>());
    if (vm.count("code")) code = vm["code"].as<string>();
    if (vm.count("codefile")) codeFile = vm["codefile"].as<string>();
    ...
    if (!codeFile.empty()) {
        ...
        if (codeFile == "-")
            ...
            code = dev::contentsString(codeFile);
        code.erase(code.find_last_not_of(" \t\n\r") + 1);  // Right trim.
    }

    unique_ptr<dev::eth::SealEngineFace> se(
        dev::eth::ChainParams(genesisInfo(networkName)).createSealEngine());
    LastBlockHashes lastBlockHashes;
    dev::eth::EnvInfo const envInfo(
    ...
    if (!code.empty()) {
        ...
        bytes codeBytes;
        try {
            codeBytes = fromHex(code, dev::WhenError::Throw);
    ...
    dev::eth::Executive executive(state, envInfo, *se);
    ...
    if (mode == Mode::Statistics) {
        onOp = [&](uint64_t,
                   ...
                   bigint gasCost,
                   bigint,
                   dev::eth::VMFace const*,
                   dev::eth::ExtVMFace const*) {
          byte b = static_cast<byte>(inst);
          counts[b].first++;
          counts[b].second += gasCost;
          total++;
          if (m > memTotal) memTotal = m;
        };
    ...
    if (mode == Mode::Statistics) {
        ...
        dev::eth::LogEntries logs = executive.logs();
        cout << logs.size() << " logs" << (logs.empty() ? "." : ":") << "\n";
        for (dev::eth::LogEntry const& l : logs) {
        ...
        for (auto const inst :
            ...
            if (count.first != 0)
                cout << "  " << instructionInfo(inst).name << " x " << count.first << " (" << count.second
                     << " gas)\n";
        }
    } else if (mode == Mode::Trace) {
        if (styledJson) {
            ...
            for (auto const& traceOp : traceJson) cout << writer.write(traceOp);
        }
        ...
        auto exception = res.excepted != dev::eth::TransactionException::None;
        cout << "output: '" << toHex(output) << "'\n";
